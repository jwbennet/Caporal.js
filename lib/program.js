...
class Program extends GetterSetter {
  ...
  _help(cmdArr = []) {
    ...
    while(cmdArr.length) {
      commandArr.push(cmdArr.shift());
      const cmdStr = commandArr.join(' ');
      cmd = parent.getCommands().filter(c => (c.name() === cmdStr || c.getAlias() === cmdStr))[0];
      if(cmd) {
        if(cmd.getCommands().length) {
          parent = cmd;
          commandArr = [];
        } else {
          break;
    ...
    if(cmd || commandArr.length === 0) {
      const help = this._helper.get(cmd);
      console.log(help);
      return help;
    } else {
      console.error('Command "' + commandArr.join(' ') + '" not found')
      return undefined;
    }
  }
  ...
  _run(args, options) {
    ...
    if (args[0] === 'help') {
      const helpText = this._help(args.slice(1));
      return process.exit(helpText ? 0 : 1);
    }
  ...
  bin() {
    return this._name ? this._name : this._bin;
  }

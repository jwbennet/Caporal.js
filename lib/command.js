...
class Command extends GetterSetter {
  ...
  constructor(name, description, program) {
    ...
    this._helper = program._helper;
    this._autocomplete = program._autocomplete;
  ...
  bin() {
    return `${this._program.bin()} ${this.name()}`;
  }
   ...
   */
  fatalError(error) {
    this._program.fatalError(error);
  }
   ...
   */
  _run(args, options) {
    if (!this._action && !this._commands.length) {
    ...
    } else if (this._commands.length) {
      this._help([ this.name() ]);
      process.exit(0);
    }
  ...
  _help(cmdArr) {
    return this._program._help(cmdArr);
  }

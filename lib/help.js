...
class Help {
  ...
  _addCustomHelp(help, options, command) {
    ...
    const key = command ? command.bin() : '_program';
  ...
  get(command) {
    ...
    if (command)  {
      help = `
   ${command.description()} ${command._name === '_default' ? this._renderHelp('_program') : this._renderHelp(command.bin())}

   ${this._getUsage(command)}`;
    } else {
      help = `
   ${chalk.cyan(this._program.name() || this._program.bin())} ${chalk.dim(this._program.version())} ${description} ${this._renderHelp('_program')}

   ${this._getUsage(command)}`;
    }

    if (!command || command.name() === '' && this._program._commands.length > 1 || command.getCommands().length) {
      help += "\n\n   " + this._getCommands(command && command.getCommands().length ? command : this._program);
    }
  ...
  _getCommandHelp(cmd) {
    ...
    let help = (cmd.bin() ? cmd.bin() + ' ' : '') + args.map(a => a.synopsis()).join(' ');

    //help += this._renderHelp(cmd.bin());
  ...
  _getUsage(cmd) {
    ...
    if (cmd && cmd.getCommands().length) {
      help += colorize(`${chalk.italic(cmd.bin())} <command> [options]`);
    } else if (cmd) {
      help += colorize(this._getCommandHelp(cmd));
    } else {
      help += colorize(`${chalk.italic(this._program.bin())} <command> [options]`);
    }
    return help;
  }
